...
import { Trash2, FileText, FlaskRoundIcon as Flask, Droplet } from "lucide-react"
...
export function WasteStats() {
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Metal & Glass</CardTitle>
          <Droplet className="h-4 w-4 text-green-500" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{metalCount + glassCount}</div>
          <p className="text-xs text-muted-foreground">
            {Math.round(((metalCount + glassCount) / totalItems) * 100)}% of total waste
          </p>
        </CardContent>
      </Card>
    </>
